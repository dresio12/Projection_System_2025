...
REZ_W <- read.csv("pred_W.csv", stringsAsFactors = FALSE, check.names = FALSE)


#create Source column for REZ dfs
REZ_W <- REZ_W |>
  mutate(Source = "REZ1") |>
  select(PlayerName, playerid, predicted_W, Source) |>
  rename(W = predicted_W)


#remove special characters
projections <- projections |>
  select(playerid, name, W, Source) |>
...
REZ_W$playerid <- as.character(REZ_W$playerid)

#comparisons df
comparisons <- bind_rows(REZ_W, projections) |>
  arrange(name, desc(W), Source)
...
#averaging all sources together to create final W prediction
averages <- bind_rows(REZ_W, projections) |>
  arrange(name, desc(W), Source)
...
newW <- averages %>%
  ...
  summarize(
    new_W = weighted.mean(W, weight),  # Always use weighted mean
    .groups = "drop"
  )

newW <- left_join(newW, REZ_W, by = c("name", "playerid"))

newW <- newW |>
  mutate(
    W_adj_rate = new_W / W,
    W_adj_rate = ifelse(W_adj_rate == 0, 1, W_adj_rate)  # Prevent division by zero
  )

write.csv(newW, "newW.csv" , row.names = FALSE)
