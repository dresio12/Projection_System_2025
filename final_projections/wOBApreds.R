...
formula <- as.formula(paste("wOBA ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$wOBA
test_labels  <- batspredict$wOBA
...
batspredict <- batspredict |> mutate(predicted_wOBA = round(xgb_predictions, 3))
...
pred_wOBA <- left_join(batspredict, teams)
...
pred_wOBA <- left_join(pred_wOBA, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_wOBA, "wOBA_preds.csv", row.names = FALSE)
