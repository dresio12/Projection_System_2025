...
REZ_GS <- read.csv("pred_pGS.csv", stringsAsFactors = FALSE, check.names = FALSE)


#create Source column for REZ dfs
REZ_GS <- REZ_GS |>
  mutate(Source = "REZ1") |>
  select(PlayerName, playerid, predicted_GS, Source) |>
  rename(GS = predicted_GS)


#remove special characters
projections <- projections |>
  select(playerid, name, GS, Source) |>
...
REZ_GS$playerid <- as.character(REZ_GS$playerid)

#comparisons df
comparisons <- bind_rows(REZ_GS, projections) |>
  arrange(name, desc(GS), Source)
...
#averaging all sources together to create final GS prediction
averages <- bind_rows(REZ_GS, projections) |>
  arrange(name, desc(GS), Source)
...
newGS <- averages %>%
  ...
  summarize(
    new_GS = weighted.mean(GS, weight),  # Always use weighted mean
    .groups = "drop"
  )

newGS <- left_join(newGS, REZ_GS, by = c("name", "playerid"))

newGS <- newGS |>
  mutate(
    GS_adj_rate = new_GS / GS,
    GS_adj_rate = ifelse(GS_adj_rate == 0, 1, GS_adj_rate)  # Prevent division by zero
  )

write.csv(newGS, "newpGS.csv" , row.names = FALSE)
