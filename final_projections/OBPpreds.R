...
formula <- as.formula(paste("OBP ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$OBP
test_labels  <- batspredict$OBP
...
batspredict <- batspredict |> mutate(predicted_OBP = round(xgb_predictions, 3))
...
pred_OBP <- left_join(batspredict, teams)
...
pred_OBP <- left_join(pred_OBP, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_OBP, "OBP_preds.csv", row.names = FALSE)
