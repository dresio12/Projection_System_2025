...
formula <- as.formula(paste("TBF ~", paste(paste0("`", predictors, "`"), collapse = " + ")))
...
train_labels <- pitchtrain$TBF
test_labels  <- pitchpredict$TBF
...
pitchpredict <- pitchpredict |> mutate(predicted_TBF = round(xgb_predictions, 3))

pitchpredict$playerid <- as.character(pitchpredict$playerid)

pred_TBF <- pitchpredict 

only_preds <- pred_TBF |>
  select(PlayerName, playerid, Throws, predicted_TBF)

#plotting

# Select relevant columns for output
boostedTBF5a <- pred_TBF |>
  select(PlayerName, predicted_TBF, TBF_Y1, TBF_Y2, TBF_Y3)

boostedTBF5a$meanTBF <- rowMeans(boostedTBF5a[, c("TBF_Y1", "TBF_Y2", "TBF_Y3")], na.rm = TRUE)


#plot
ggplot(boostedTBF5a, aes(x = predicted_TBF, y = TBF_Y1)) +
  ...
  labs(
    title = "2025 Predicted TBF vs 2024 TBF",
    x = "2025 Predicted TBF",
    y = "2024 TBF"
  ) +
  theme_minimal()


ggplot(boostedTBF5a, aes(x = predicted_TBF, y = meanTBF)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted TBF vs 3-Year Mean TBF",
    x = "2025 Predicted TBF",
    y = "Mean TBF"
  ) +
  theme_minimal()
...
#plot again, this time with players with meaningful 2024 TBF

boostedTBF5b <- pred_TBF |>
  filter(IP_Y1 >= 20) |>
  select(PlayerName, predicted_TBF, TBF_Y1, TBF_Y2, TBF_Y3)

boostedTBF5b$meanTBF <- rowMeans(boostedTBF5b[, c("TBF_Y1", "TBF_Y2", "TBF_Y3")], na.rm = TRUE)


ggplot(boostedTBF5b, aes(x = predicted_TBF, y = TBF_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted TBF vs 2024 TBF",
...
ggplot(boostedTBF5b, aes(x = predicted_TBF, y = meanTBF)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted TBF vs 3-Year Mean TBF",
...
#plot again, increase TBF

boostedTBF5c <- pred_TBF |>
  filter(IP_Y1 >= 50) |>
  select(PlayerName, predicted_TBF, TBF_Y1, TBF_Y2, TBF_Y3) 

boostedTBF5c$meanTBF <- rowMeans(boostedTBF5c[, c("TBF_Y1", "TBF_Y2", "TBF_Y3")], na.rm = TRUE)


ggplot(boostedTBF5c, aes(x = predicted_TBF, y = TBF_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted TBF vs 2024 TBF",
...
ggplot(boostedTBF5c, aes(x = predicted_TBF, y = meanTBF)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted TBF vs 3-Year Mean TBF",
...
write.csv(only_preds, "pred_TBF.csv", row.names = FALSE)
