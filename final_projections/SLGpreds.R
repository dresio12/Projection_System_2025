...
formula <- as.formula(paste("SLG ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$SLG
test_labels  <- batspredict$SLG
...
batspredict <- batspredict |> mutate(predicted_SLG = round(xgb_predictions, 3))
...
pred_SLG <- left_join(batspredict, teams)
...
pred_SLG <- left_join(pred_SLG, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_SLG, "SLG_preds.csv", row.names = FALSE)
