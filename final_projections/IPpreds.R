...
formula <- as.formula(paste("IP ~", paste(paste0("`", predictors, "`"), collapse = " + ")))
...
train_labels <- pitchtrain$IP
test_labels  <- pitchpredict$IP
...
pitchpredict <- pitchpredict |> mutate(predicted_IP = round(xgb_predictions, 3))

pitchpredict$playerid <- as.character(pitchpredict$playerid)

pred_IP <- pitchpredict 

only_preds <- pred_IP |>
  select(PlayerName, playerid, Throws, predicted_IP)

#plotting

# Select relevant columns for output
boostedIP5a <- pred_IP |>
  select(PlayerName, predicted_IP, IP_Y1, IP_Y2, IP_Y3)

boostedIP5a$meanIP <- rowMeans(boostedIP5a[, c("IP_Y1", "IP_Y2", "IP_Y3")], na.rm = TRUE)


#plot
ggplot(boostedIP5a, aes(x = predicted_IP, y = IP_Y1)) +
  ...
  labs(
    title = "2025 Predicted IP vs 2024 IP",
    x = "2025 Predicted IP",
    y = "2024 IP"
  ) +
  theme_minimal()
...
ggplot(boostedIP5a, aes(x = predicted_IP, y = meanIP)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted IP vs 3-Year Mean IP",
    x = "2025 Predicted IP",
    y = "Mean IP"
  ) +
  theme_minimal()
...
#plot again, this time with players with meaningful 2024 IP

boostedIP5b <- pred_IP |>
  filter(IP_Y1 >= 20) |>
  select(PlayerName, predicted_IP, IP_Y1, IP_Y2, IP_Y3)

boostedIP5b$meanIP <- rowMeans(boostedIP5b[, c("IP_Y1", "IP_Y2", "IP_Y3")], na.rm = TRUE)


ggplot(boostedIP5b, aes(x = predicted_IP, y = IP_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted IP vs 2024 IP",
...
ggplot(boostedIP5b, aes(x = predicted_IP, y = meanIP)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted IP vs 3-Year Mean IP",
...
#plot again, increase IP

boostedIP5c <- pred_IP |>
  filter(IP_Y1 >= 50) |>
  select(PlayerName, predicted_IP, IP_Y1, IP_Y2, IP_Y3) 

boostedIP5c$meanIP <- rowMeans(boostedIP5c[, c("IP_Y1", "IP_Y2", "IP_Y3")], na.rm = TRUE)


ggplot(boostedIP5c, aes(x = predicted_IP, y = IP_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted IP vs 2024 IP",
...
ggplot(boostedIP5c, aes(x = predicted_IP, y = meanIP)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted IP vs 3-Year Mean IP",
...
write.csv(only_preds, "pred_IP.csv", row.names = FALSE)
