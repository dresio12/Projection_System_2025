...
formula <- as.formula(paste("BB_pct ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$BB_pct
test_labels  <- batspredict$BB_pct
...
batspredict <- batspredict |> mutate(predicted_BB_pct = round(xgb_predictions, 3))
...
pred_BB_pct <- left_join(batspredict, teams)
...
pred_BB_pct <- left_join(pred_BB_pct, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_BB_pct, "BB_pct_preds.csv", row.names = FALSE)
