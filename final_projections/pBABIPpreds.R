...
formula <- as.formula(paste("BABIP ~", paste(paste0("`", predictors, "`"), collapse = " + ")))
...
train_labels <- pitchtrain$BABIP
test_labels  <- pitchpredict$BABIP
...
pitchpredict <- pitchpredict |> mutate(predicted_BABIP = round(xgb_predictions, 3))

pitchpredict$playerid <- as.character(pitchpredict$playerid)

pred_BABIP <- pitchpredict 

only_preds <- pred_BABIP |>
  select(PlayerName, playerid, Throws, predicted_BABIP)

#plotting

# Select relevant columns for output
boostedBABIP5a <- pred_BABIP |>
  select(PlayerName, predicted_BABIP, BABIP_Y1, BABIP_Y2, BABIP_Y3)

boostedBABIP5a$meanBABIP <- rowMeans(boostedBABIP5a[, c("BABIP_Y1", "BABIP_Y2", "BABIP_Y3")], na.rm = TRUE)


#plot
ggplot(boostedBABIP5a, aes(x = predicted_BABIP, y = BABIP_Y1)) +
  ...
  labs(
    title = "2025 Predicted BABIP vs 2024 BABIP",
    x = "2025 Predicted BABIP",
    y = "2024 BABIP"
  ) +
  theme_minimal()


ggplot(boostedBABIP5a, aes(x = predicted_BABIP, y = meanBABIP)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted BABIP vs 3-Year Mean BABIP",
    x = "2025 Predicted BABIP",
    y = "Mean BABIP"
  ) +
  theme_minimal()
...
#plot again, this time with players with meaningful 2024 BABIP

boostedBABIP5b <- pred_BABIP |>
  filter(IP_Y1 >= 20) |>
  select(PlayerName, predicted_BABIP, BABIP_Y1, BABIP_Y2, BABIP_Y3)

boostedBABIP5b$meanBABIP <- rowMeans(boostedBABIP5b[, c("BABIP_Y1", "BABIP_Y2", "BABIP_Y3")], na.rm = TRUE)


ggplot(boostedBABIP5b, aes(x = predicted_BABIP, y = BABIP_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted BABIP vs 2024 BABIP",
...
ggplot(boostedBABIP5b, aes(x = predicted_BABIP, y = meanBABIP)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted BABIP vs 3-Year Mean BABIP",
...
#plot again, increase BABIP

boostedBABIP5c <- pred_BABIP |>
  filter(IP_Y1 >= 50) |>
  select(PlayerName, predicted_BABIP, BABIP_Y1, BABIP_Y2, BABIP_Y3) 

boostedBABIP5c$meanBABIP <- rowMeans(boostedBABIP5c[, c("BABIP_Y1", "BABIP_Y2", "BABIP_Y3")], na.rm = TRUE)


ggplot(boostedBABIP5c, aes(x = predicted_BABIP, y = BABIP_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted BABIP vs 2024 BABIP",
...
ggplot(boostedBABIP5c, aes(x = predicted_BABIP, y = meanBABIP)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted BABIP vs 3-Year Mean BABIP",
...
write.csv(only_preds, "pred_pBABIP.csv", row.names = FALSE)
