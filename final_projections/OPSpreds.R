...
formula <- as.formula(paste("OPS ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$OPS
test_labels  <- batspredict$OPS
...
batspredict <- batspredict |> mutate(predicted_OPS = round(xgb_predictions, 3))
...
pred_OPS <- left_join(batspredict, teams)
...
pred_OPS <- left_join(pred_OPS, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_OPS, "OPS_preds.csv", row.names = FALSE)
