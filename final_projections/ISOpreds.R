...
formula <- as.formula(paste("ISO ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$ISO
test_labels  <- batspredict$ISO
...
batspredict <- batspredict |> mutate(predicted_ISO = round(xgb_predictions, 3))
...
pred_ISO <- left_join(batspredict, teams)
...
pred_ISO <- left_join(pred_ISO, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_ISO, "ISO_preds.csv", row.names = FALSE)
