...
formula <- as.formula(paste("SV ~", paste(paste0("`", predictors, "`"), collapse = " + ")))
...
train_labels <- pitchtrain$SV
test_labels  <- pitchpredict$SV
...
pitchpredict <- pitchpredict |> mutate(predicted_SV = round(xgb_predictions, 3))

pitchpredict$playerid <- as.character(pitchpredict$playerid)

pred_SV <- pitchpredict 

only_preds <- pred_SV |>
  select(PlayerName, playerid, Throws, predicted_SV)

#plotting

# Select relevant columns for output
boostedSV5a <- pred_SV |>
  select(PlayerName, predicted_SV, SV_Y1, SV_Y2, SV_Y3)

boostedSV5a$meanSV <- rowMeans(boostedSV5a[, c("SV_Y1", "SV_Y2", "SV_Y3")], na.rm = TRUE)


#plot
ggplot(boostedSV5a, aes(x = predicted_SV, y = SV_Y1)) +
  ...
  labs(
    title = "2025 Predicted SV vs 2024 SV",
    x = "2025 Predicted SV",
    y = "2024 SV"
  ) +
  theme_minimal()
...
ggplot(boostedSV5a, aes(x = predicted_SV, y = meanSV)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted SV vs 3-Year Mean SV",
    x = "2025 Predicted SV",
    y = "Mean SV"
  ) +
  theme_minimal()
...
#plot again, this time with players with meaningful 2024 SV

boostedSV5b <- pred_SV |>
  filter(IP_Y1 >= 20) |>
  select(PlayerName, predicted_SV, SV_Y1, SV_Y2, SV_Y3)

boostedSV5b$meanSV <- rowMeans(boostedSV5b[, c("SV_Y1", "SV_Y2", "SV_Y3")], na.rm = TRUE)


ggplot(boostedSV5b, aes(x = predicted_SV, y = SV_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted SV vs 2024 SV",
...
ggplot(boostedSV5b, aes(x = predicted_SV, y = meanSV)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted SV vs 3-Year Mean SV",
...
#plot again, increase SV

boostedSV5c <- pred_SV |>
  filter(IP_Y1 >= 50) |>
  select(PlayerName, predicted_SV, SV_Y1, SV_Y2, SV_Y3) 

boostedSV5c$meanSV <- rowMeans(boostedSV5c[, c("SV_Y1", "SV_Y2", "SV_Y3")], na.rm = TRUE)


ggplot(boostedSV5c, aes(x = predicted_SV, y = SV_Y1)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted SV vs 2024 SV",
...
ggplot(boostedSV5c, aes(x = predicted_SV, y = meanSV)) +
  geom_point(color = "blue") +  # Plot the points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +  # Add a reference line (ideal prediction)
  labs(
    title = "2025 Predicted SV vs 3-Year Mean SV",
...
write.csv(only_preds, "pred_SV.csv", row.names = FALSE)
