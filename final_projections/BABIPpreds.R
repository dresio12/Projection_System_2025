...
formula <- as.formula(paste("BABIP ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$BABIP
test_labels  <- batspredict$BABIP
...
batspredict <- batspredict |> mutate(predicted_BABIP = round(xgb_predictions, 3))
...
pred_BABIP <- left_join(batspredict, teams)
...
pred_BABIP <- left_join(pred_BABIP, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_BABIP, "BABIP_preds.csv", row.names = FALSE)
