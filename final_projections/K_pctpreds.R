...
formula <- as.formula(paste("K_pct ~", paste(predictors, collapse = " + ")))
...
train_labels <- batstrain$K_pct
test_labels  <- batspredict$K_pct
...
batspredict <- batspredict |> mutate(predicted_K_pct = round(xgb_predictions, 3))
...
pred_K_pct <- left_join(batspredict, teams)
...
pred_K_pct <- left_join(pred_K_pct, pfs) |>
  select(name, playerid, Age, Bats, 327:339) |>
  select(1:6)

write.csv(pred_K_pct, "K_pct_preds.csv", row.names = FALSE)
