...
train_labels <- batstrain$BaseRunning
test_labels  <- batspredict$BaseRunning
...
batspredict <- batspredict |> mutate(predicted_BaseRunning = round(xgb_predictions, 3))
...
pred_BaseRunning <- left_join(batspredict, teams)
...
pred_BaseRunning <- left_join(pred_BaseRunning, pfs) |>
  select(name, playerid, Age, Bats, 327:339)


newPA <- read.csv("newPA.csv")

pred_BaseRunning <- left_join(pred_BaseRunning, newPA)
pred_BaseRunning <- pred_BaseRunning |>
  mutate(predicted_BaseRunning = round(predicted_BaseRunning * new_PA / 600),
         new_PA = round(new_PA)) |>
  select(1:6, 18)

write.csv(pred_BaseRunning, "BaseRunning_preds.csv", row.names = FALSE)
